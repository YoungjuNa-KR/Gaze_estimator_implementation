...
def loadLabel(lines, names):
    ...
    for name in names:
        ...
        batch_image_person_number = name.split("_")[0][1:]
        batch_image_number = int(name.split(".")[0].split("_")[1])  
        if batch_image_person_number == '00':
            min = 0
            max = 2999 + 3000 * 0
            mid = int((min+max)/2)

        elif batch_image_person_number == '01':
            min = 3000 * 1
            max = 2999 + 3000 * 1
            mid = int((min+max)/2)

        elif batch_image_person_number == '02':
            min = 3000 * 2
            max = 2999 + 3000 * 2
            mid = int((min+max)/2)
        
        elif batch_image_person_number == '03':
            min = 3000 * 3
            max = 2999 + 3000 * 3
            mid = int((min+max)/2)

        elif batch_image_person_number == '04':
            min = 3000 * 4
            max = 2999 + 3000 * 4
            mid = int((min+max)/2)
        
        elif batch_image_person_number == '05':
            min = 3000 * 5
            max = 2999 + 3000 * 5
            mid = int((min+max)/2)
        
        elif batch_image_person_number == '06':
            min = 3000 * 6
            max = 2999 + 3000 * 6
            mid = int((min+max)/2)
        
        elif batch_image_person_number == '07':
            min = 3000 * 7
            max = 2999 + 3000 * 7
            mid = int((min+max)/2)

        elif batch_image_person_number == '08':
            min = 3000 * 8
            max = 2999 + 3000 * 8
            mid = int((min+max)/2)

        elif batch_image_person_number == '09':
            min = 3000 * 9
            max = 2999 + 3000 * 9
            mid = int((min+max)/2)
        ...
        while True:
            ...
            if batch_image_number == image_number:
                ...
                head_batch_label.append([float(label[3]),float(label[4])])
                gaze_batch_label.append([float(label[1]),float(label[2])])
                break
    ...
    for i in range(len(sr_batch_size)):
        ...
        for line in lines:
            if image_names[i] in line:
                left_y = int(line.split(",")[2]) // 2
                left_x = int(line.split(",")[4]) // 2
                right_y =int(line.split(",")[1]) // 2
                right_x =int(line.split(",")[3]) // 2
                flag = True
